...
export default function PaymentsModal({
  ...
  return (
    <Modal
      ...
      position="center"
      className="max-w-[648px] w-full rounded-lg lg:rounded-xl overflow-hidden mx-3"
    >
      <ModalHeader
        label="Formas de pagamento"
        onClose={() => setIsModalPaymentsVisible(false)}
      />
      <div className="p-9">
        <div className="border-b border-background-softLight w-full flex gap-10 lg:gap-12 mb-10 relative">
          <button
        ...
        {modalPaymentSelected === PaymentMethods.ticket && (
          <div>
            ...
            <p className="text-xs lg:text-sm mt-5">
              ...
              úteis para ter o pagamento confirmado. A prazo de entrega começa a
              ser contado a partir da data de confirmação do pagamento.
            </p>
          </div>
        )}
        {modalPaymentSelected === PaymentMethods.pix && (
          <div>
            ...
            <p className="text-xs lg:text-sm mt-5">
              O Pix é gerado automaticamente no final da compra, basta escanear
              o QR Code ou copiar o código para realizar o pagamento. A
              aprovação pode levar até 15 segundos.
            </p>
          </div>
        )}
      </div>
